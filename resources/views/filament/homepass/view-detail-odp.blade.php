        ...
        <tr>
            ...
            <tr>
                <td>
                    ...
                    <div>
                        <label class="block text-sm font-medium ">ODP Name</label>
                    ...
                    <button type="button" 
                        @click.prevent="
                            fetch('/api/rotate-photo', { 
                                method: 'POST', 
                                headers: {'Content-Type':'application/json'}, 
                                body: JSON.stringify({ path: '{{ $record->odp_terbuka }}' }) 
                            })
                            .then(res => res.json())
                            .then(res => {
                                if(res.success){
        ...
        <tr>
            ...
            <td style="padding:3px; width:50%;">
                <div style="
                    ...
                    overflow:hidden;
                ">
                    @if ($record->odp_terbuka)
                        <img id="foto2" src="{{ asset('storage/'.$record->odp_terbuka) }}?v={{ now()->timestamp }}" 
                    ...
                    <button type="button" 
                        @click.prevent="
                            fetch('/api/rotate-photo', { 
                                method: 'POST', 
                                headers: {'Content-Type':'application/json'}, 
                                body: JSON.stringify({ path: '{{ $record->odp_tertutup }}' }) 
                            })
                            .then(res => res.json())
                            .then(res => {
                                if(res.success){
                    ...
                    <button type="button" 
                        @click.prevent="
                            fetch('/api/rotate-photo', { 
                                method: 'POST', 
                                headers: {'Content-Type':'application/json'}, 
                                body: JSON.stringify({ path: '{{ $record->power_optic_odc }}' }) 
                            })
                            .then(res => res.json())
                            .then(res => {
                                if(res.success){
        ...
        <tr>
            <td style="padding:8px; width:50%;">
                <div style="
                    ...
                    overflow:hidden;
                ">
                    @if ($record->odp_tertutup)
                        <img id="foto3" src="{{ asset('storage/'.$record->odp_tertutup) }}?v={{ now()->timestamp }}" 
                        style="width:100%;height:100%;object-fit:contain;">
            ...
            <td style="padding:8px; width:50%;">
                <div style="
                    ...
                    overflow:hidden;
                ">
                    @if ($record->power_optic_odc)
                        <img id="foto4" src="{{ asset('storage/'.$record->power_optic_odc) }}?v={{ now()->timestamp }}" 
                        style="width:100%;height:100%;object-fit:contain;">
